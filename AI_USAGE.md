...
## 2026-08-24 — Codex（Recognition API error handling）

Codexが画像upload時の非JSON応答を無条件に`response.json()`へ渡していた問題を修正しました。production API URLの`/api`重複を防ぐURL結合、Content-Type・HTTP status・RecognitionResult形式の検証、FastAPI error detailの表示を追加し、Render/Vercel環境変数の設定方法を明確化しました。Recognitionアルゴリズムは変更していません。

## 2026-08-24 — Codex（UI design system polish）

Codexが既存の白・アイボリー・深緑・オレンジの方向性を維持し、色、spacing、radius、shadow、typographyをCSS tokenへ統合しました。Editor node、transition、Card、Button、Input、Badge、Alert、Recognition Review、レスポンシブ配置を同じ視覚ルールへ揃え、既存E2Eを実行して画面キャプチャを目視確認しました。機能とRecognitionアルゴリズムは変更していません。
