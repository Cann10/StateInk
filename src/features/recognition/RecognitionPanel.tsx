...
export function RecognitionPanel({ onConfirm, onClose }: Props) {
  ...
  const runRecognition = async (file: File) => {
    ...
    try {
      ...
      setError(timedOut
        ? `認識サーバーが${RECOGNITION_TIMEOUT_MS / 1000}秒以内に応答しませんでした。通信環境を確認して再試行してください。`
        : reason instanceof Error ? reason.message : '画像を読み取れませんでした');
    } finally {
  ...
  const focusCandidate = (id: string) => {
    ...
    document.getElementById(`review-${id}`)?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  };
  ...
  const resetReview = () => { setResult(undefined); setImageUrl(undefined); setActiveCandidateId(undefined); };
  ...
  return <section className="recognition card" aria-label="画像読み取りレビュー">
    ...
    {!result && <div className="upload"><ImagePlus size={34}/><strong>状態遷移図の画像を選択</strong><p>認識は自動完成ではなく、編集できる下書きを作ります。日本語・英語の文字と、白紙に黒ペンで描いた図を読み取ります。</p><label className="primary">画像を選択<input aria-label="状態遷移図の画像" type="file" accept="image/png,image/jpeg,image/webp" onChange={recognize}/></label>{loading && <div className="loading-state" role="status"><LoaderCircle className="spin"/>下書きを作成しています…</div>}{error && <div className="upload-error" role="alert"><strong>画像を読み取れませんでした</strong><p>{error}</p><p>バックエンドの起動と画像形式を確認してください。</p>{lastFile && <button type="button" className="primary" disabled={loading} onClick={() => void runRecognition(lastFile)}>同じ画像で再試行</button>}</div>}</div>}
    {result && <div className="review"><div className="review-notice"><AlertTriangle/><div><strong>読み取り結果を確認してください</strong><p>元画像と色付き候補を見比べ、名前・接続・向きを確認してください（{result.processing_ms.toFixed(0)} ms）。</p></div></div>{result.warnings.map((warning) => <p className="recognition-warning" key={warning}>{warning}</p>)}
      ...
      <div className="review-actions"><button className="reset" onClick={resetReview}>別の画像を選ぶ</button><button className="primary" disabled={result.states.length === 0 || !result.states.some((state) => state.initial) || unconfirmedCount > 0} onClick={() => onConfirm(recognitionToStateMachine(result))}>確認してEditorへ</button></div>
    </div>}
  </section>;
}
