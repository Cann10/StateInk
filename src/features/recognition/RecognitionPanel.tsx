...
export function RecognitionPanel({ onConfirm, onClose }: Props) {
  ...
  return <section className="recognition card" aria-label="画像読み取りレビュー">
    <div className="card-title"><div><span>RECOGNITION ・ 読み取りレビュー</span><h2 ref={headingRef} tabIndex={-1}>写真から下書きを作る</h2><p className="card-hint">STEP 1〜2。読み取り結果は「下書き」です。ここで直してから Editor へ渡します。</p></div><button className="reset" onClick={onClose}><X size={16}/>閉じる</button></div>
