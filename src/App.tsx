...
export function App() {
  ...
  const confirmDiscard = () => !machineIsUserWork
    || window.confirm('編集中の図が失われます。先に Export で保存できます。破棄してよろしいですか？');
  ...
  if (screen === 'home') return <main className="home-screen">
    ...
    <section className="home-start"><div><p className="eyebrow">START</p><h2>3つの入り口から選べます</h2><p className="home-start-hint">初めての方は <b>「サンプルを試す」</b> がおすすめです。</p></div><nav className="start-options" aria-label="はじめる方法">
    ...
    <section className="home-steps"><p className="eyebrow">HOW IT WORKS</p><div><article><b>1</b><strong>読み込む</strong><span>写真・新規・サンプル</span></article><article><b>2</b><strong>確認する</strong><span>読み取り結果をチェック</span></article><article><b>3</b><strong>編集する</strong><span>状態と矢印を直す</span></article><article><b>4</b><strong>動かす</strong><span>イベントを押して確認</span></article></div></section>
  </main>;
  ...
  return <main>
    ...
    <section className="intro"><h2>{sampleKey === 'broken' ? 'まず問題を再現し、refund 遷移を追加して直してみましょう。' : '図を直すと、チェック結果もすぐに変わります。'}</h2><p>{sampleKey === 'broken' ? 'coin → select → sold_out を押すと問題状態へ進みます。売り切れから待機への refund を追加すると警告が消えます。' : '状態を選んで編集するか、下のフォームで遷移を追加してください。編集するとシミュレーターは開始地点へ戻ります。'}</p></section>
    <div className="workspace" id="main-content" tabIndex={-1}>
      <section className="card diagram"><div className="card-title"><div><span>EDITOR</span><h2>状態遷移図</h2><p className="card-hint">状態（丸）と矢印を足したり直したりします。図の中の状態・矢印をクリックすると、下で名前を編集できます。</p></div><button className="primary small" onClick={() => { const next = addState(machine, '新しい状態', { x: 100 + machine.states.length * 25, y: 300 }); applyEdit(() => next); setSelectedStateId(next.states.at(-1)?.id); }}><CirclePlus size={16}/> 状態を追加</button></div>
      ...
      <aside><section className="card simulator" ref={simulatorRef}><div className="card-title"><div><span>SIMULATOR</span><h2>動作を試す</h2><p className="card-hint">イベントのボタンを押すと、図がその通りに動きます。</p></div><button className="reset" onClick={() => { setSimulation(reset(machine)); setReplayedPath(undefined); }}><RotateCcw size={15}/>リセット</button></div>{simulation.error ? <div className="blocked" role="status"><AlertTriangle aria-hidden="true"/><div>{initialErrorText[simulation.error]}<span className="blocked-fix">図で開始状態（▶）をちょうど1つ設定してください。</span></div></div> : <><p className="caption">現在の状態</p><div className="current" aria-live="polite"><Play size={18} fill="currentColor"/><strong>{current?.name}</strong></div>{replayedPath && <p className="replay-status" role="status">最短操作列を再現し、経路を図で強調しています。</p>}<p className="caption">利用可能な操作</p><div className="events">{availableEvents(machine, simulation).map((event) => <button key={event} onClick={() => { setReplayedPath(undefined); setSimulation((previous) => transition(machine, previous, event)); }}>{event}<span>→</span></button>)}{availableEvents(machine, simulation).length === 0 && <p className="empty">この状態から進めるイベントがありません。行き止まりでなければ、Editorで遷移を追加してください。</p>}</div><p className="caption">操作の履歴（Trace）</p><ol className="trace">{simulation.trace.map((step, index) => <li key={`${index}-${step.stateId}`}>{step.event && <><b>{step.event}</b><span>→</span></>}<strong>{machine.states.find((state) => state.id === step.stateId)?.name}</strong></li>)}</ol></>}</section>
        <section className="card analysis"><div className="card-title"><div><span>ANALYZER</span><h2>設計チェック</h2><p className="card-hint">編集するたびに、行き止まり・到達不能などの問題を自動で調べます。</p></div><span className="live">● 自動更新</span></div><div className="analysis-body" aria-live="polite">{shownIssues.length === 0 ? <div className="success"><CheckCircle2/><div><strong>問題は見つかりませんでした</strong><p>編集内容はすぐにチェックされています。</p></div></div> : <><div className="issue-count"><AlertTriangle/><strong>確認したい問題が{shownIssues.length}件あります</strong></div>{shownIssues.map((item, index) => <article className="issue" key={`${item.type}-${index}`}><h3>{item.title}</h3><p className="issue-desc"><span className="issue-label">どうなる？</span>{item.description}</p>{item.counterexample && item.counterexample.length > 0 ? <div className="reproduce"><span>問題を再現する操作</span><strong>{item.counterexample.join(' → ')}</strong><button onClick={() => replayCounterexample(item.counterexample ?? [])}>問題を再現</button></div> : item.type === 'unreachable' || item.type === 'isolated' ? <p className="no-path">初期状態から到達できないため、再現操作はありません。</p> : null}{item.suggestions?.map((suggestion) => <div className="fix-suggestion" key={`${suggestion.from}-${suggestion.to}-${suggestion.event}`}><span>直し方（押すと追加）</span><strong>{suggestion.description}</strong><code>{suggestion.event}</code><button onClick={() => { applyEdit((value) => addTransition(value, suggestion.from, suggestion.to, suggestion.event)); flashNotice('修正候補を反映しました。'); }}>この候補を追加</button></div>)}<small>{item.technicalName}</small></article>)}</>}</div></section></aside>
    </div>
  </main>;
}
