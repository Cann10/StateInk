  ...
  return <main>
    ...
    <nav className="flow-steps" aria-label="このツールの進めかた">
      {[{ n: 1, label: '読み込む' }, { n: 2, label: '確認する' }, { n: 3, label: '編集する' }, { n: 4, label: '動かす' }].map((step) => (
        <span key={step.n} className="flow-step" data-state={(showRecognition ? step.n <= 2 : step.n >= 3) ? 'current' : 'idle'}><b>{step.n}</b>{step.label}</span>
      ))}
    </nav>
    ...
    <div className="workspace">
      ...
      <aside><section className="card simulator"><div className="card-title"><div><span>02 / RUN<span className="eyebrow-en"> ・ SIMULATE</span> ・ 動かす</span><h2>動作を試す</h2><p className="card-hint">イベントのボタンを押すと、図がその通りに動きます（Simulator）。</p></div><button className="reset" onClick={() => { setSimulation(reset(machine)); setReplayedPath(undefined); }}><RotateCcw size={15}/>リセット</button></div>{simulation.error ? <div className="blocked"><AlertTriangle/><div>{initialErrorText[simulation.error]}<span className="blocked-fix">図で開始状態（▶）をちょうど1つ設定してください。</span></div></div> : <><p className="caption">現在の状態</p><div className="current"><Play size={18} fill="currentColor"/><strong>{current?.name}</strong></div>{replayedPath && <p className="replay-status" role="status">最短操作列を再現し、経路を図で強調しています。</p>}<p className="caption">利用可能な操作</p><div className="events">{availableEvents(machine, simulation).map((event) => <button key={event} onClick={() => { setReplayedPath(undefined); setSimulation((previous) => transition(machine, previous, event)); }}>{event}<span>→</span></button>)}{availableEvents(machine, simulation).length === 0 && <p className="empty">この状態から進めるイベントがありません。行き止まりでなければ、Editorで遷移を追加してください。</p>}</div><p className="caption">操作の履歴（Trace）</p><ol className="trace">{simulation.trace.map((step, index) => <li key={`${index}-${step.stateId}`}>{step.event && <><b>{step.event}</b><span>→</span></>}<strong>{machine.states.find((state) => state.id === step.stateId)?.name}</strong></li>)}</ol></>}</section>
        <section className="card analysis"><div className="card-title"><div><span>03 / CHECK<span className="eyebrow-en"> ・ LIVE</span> ・ 設計チェック</span><h2>設計チェック</h2><p className="card-hint">編集するたびに、行き止まり・到達不能などの問題を自動で調べます（Analyzer）。</p></div><span className="live">● 自動更新</span></div>{shownIssues.length === 0 ? <div className="success"><CheckCircle2/><div><strong>問題は見つかりませんでした</strong><p>編集内容はすぐにチェックされています。</p></div></div> : <><div className="issue-count"><AlertTriangle/><strong>確認したい問題が{shownIssues.length}件あります</strong></div>{shownIssues.map((item, index) => <article className="issue" key={`${item.type}-${index}`}><h3>{item.title}</h3><p className="issue-desc"><span className="issue-label">どうなる？</span>{item.description}</p>{item.counterexample && item.counterexample.length > 0 ? <div className="reproduce"><span>問題を再現する操作</span><strong>{item.counterexample.join(' → ')}</strong><button onClick={() => { const replay = replayEvents(machine, item.counterexample ?? []); setSimulation(replay.simulation); setReplayedPath(replay); }}>問題を再現</button></div> : item.type === 'unreachable' || item.type === 'isolated' ? <p className="no-path">初期状態から到達できないため、再現操作はありません。</p> : null}{item.suggestions?.map((suggestion) => <div className="fix-suggestion" key={`${suggestion.from}-${suggestion.to}-${suggestion.event}`}><span>直し方（押すと追加）</span><strong>{suggestion.description}</strong><code>{suggestion.event}</code><button onClick={() => applyEdit((value) => addTransition(value, suggestion.from, suggestion.to, suggestion.event))}>この候補を追加</button></div>)}<small>{item.technicalName}</small></article>)}</>}</section></aside>
    </div>
  </main>;
}
