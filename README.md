...
OpenCVの輪郭解析で円・楕円・長方形の状態候補、Hough線分とarrowhead候補から遷移候補を作ります。OCRはTesseractがある場合だけ短い英数字を補助的に読みます。認識結果は**自動完成ではなく下書き**です。confidenceを表示し、人が状態名、イベント、source、target、方向、initial/finalを確認してからIRへ確定します。
...
- 対応: 白紙＋黒ペン、円・楕円・長方形、明確な直線矢印、短い英数字ラベル
- 苦手: 曲線・self-loop・交差線・密な図・影や強い遠近歪み・日本語OCR
...
任意でTesseractを導入すると英数字OCRを試せます。未導入でも仮名を持つ構造候補を返します。
